...
def test_proposal_json_round_trip_and_malformed_errors():
    ...
    assert stmts[0].columns[0].months == 12 and stmts[0].columns[0].end == date(2017, 3, 31)
...
def test_a_period_end_must_be_named_by_the_filing_and_match_the_label():
    ...
    Adapted at the branch merge (ADR-0054) to the trunk V3c: `end` is a date, the rule id is
    V3c_period_close, and a declared close the column's own words contradict is refused."""
    from datetime import date as _date

    page = ("CONSOLIDATED BAL ANCE SHEET as at 30th June, 2015\n(H in lac)\n"
            "As at As at\n  30/06/2015  30/06/2014\n"
            "T O TA  L  44,486.78  39,333.05\n")
    def _bs(end: _date, period: str = "FY15") -> ProposedStatement:
    ...
    good = verify_statement(_bs(_date(2015, 6, 30)), [page])
    assert good.verified, good.violations
    assert good.figures[0].period_end == _date(2015, 6, 30)

    # a March close the filing never names — the assumption this rule exists to stop
    bad = verify_statement(_bs(_date(2015, 3, 31)), [page])
    assert any(v.rule == "V3c_period_close" and "its own words say" in v.detail
               for v in bad.violations)

    # an end date in the wrong year for its label
    wrong_year = verify_statement(_bs(_date(2015, 6, 30), period="FY16"), [page])
    assert any(v.rule == "V3c_period_close" and "not in 2016" in v.detail
               for v in wrong_year.violations)
